...
enum { SchemeNorm, SchemeSel, SchemeOcc, SchemeSelOcc, SchemeStatus, SchemeTitle }; /* color schemes */
...
int
drawstatusbar(Monitor *m, char *stext)
{
	int ret, i, w, x, len;
	short isCode = 0;
	char *text;
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) {
		tw = m->ww - drawstatusbar(m, stext) - stw;
	}
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (is_selected && is_occupied)
			drw_setscheme(drw, scheme[SchemeSelOcc]);
		else if (is_selected)
			drw_setscheme(drw, scheme[SchemeSel]);
		else if (is_occupied)
			drw_setscheme(drw, scheme[SchemeOcc]);
		else
...
{
	unsigned int iconSize = systrayiconsize > 0 ? systrayiconsize : bh;
	if (i) {
		i->h = iconSize;
		if (w == h)
			i->w = iconSize;
			...
			i->w = (int) ((float)iconSize * ((float)w / (float)h));
		applysizehints(i, &(i->x), &(i->y), &(i->w), &(i->h), False);
		if (i->h > iconSize) {
			if (i->w == i->h)
				i->w = iconSize;
			else
				i->w = (int) ((float)iconSize * ((float)i->w / (float)i->h));
			i->h = iconSize;
		}
	}
}
...
updatesystray(void)
{
	...
	if (!systray) {
		...
		if (!(systray = (Systray *)calloc(1, sizeof(Systray))))
			die("fatal: could not malloc() %u bytes\n", sizeof(Systray));
		systray->win = XCreateSimpleWindow(dpy, root, x, m->by, w, iconSize, 0, 0, scheme[SchemeSel][ColBg].pixel);
	...
	x -= w;
	XMoveResizeWindow(dpy, systray->win, x, m->by, w, iconSize);
	wc.x = x; wc.y = m->by; wc.width = w; wc.height = iconSize;
	...
	XFillRectangle(dpy, systray->win, drw->gc, 0, 0, w, iconSize);
	XSync(dpy, False);
}
